...
<v-list  >
  ...
  <v-simple-table id="menu_table">
    ...
    <template v-slot:default>
      ...
      <tbody>
        <tr v-for="item in iPadItems" :key="item.name" height="100px">
...
import { dbiPadAdd } from '../../firebase'
  ...
  export default {
    data () {
     ...
     beforeCreate() {
      this.$store.dispatch('setiPadItems')
    },
    methods: {
      addCheckoutItem() {
    ...
    computed: {
      ...
      iPadItems () {
        return this.$store.getters.getiPadItems
      },
      subTotal () {
        var subCost = 0;
